...
function howMany(i, j, element) {
    ...
    // neighborvalues = [topLeft, left, bottomLeft, bottom, bottomRight, right, topRight, top]
    sum = 0;
    // for (let k =0; k<neighborvalues.length; k++) {
    //     if (neighborvalues[k] == "undefined"){
    //         console.log(undefined)
    //     }
    //     else {
    //         console.log(neighborvalues[k])
    //     }
    // }

    neighbor = [[i-1,j-1],[i,j-1]]
    console.log(neighbor)
    for (let k = 0; k<neighbor; k++) {
        if (verify(neighbor[k][0], neighbor[k][1])) {
            console.log(theDojo)
            sum +=theDojo[y][x]
            }
        }
